...
- **Password-protected** — login page with session cookies
...
- The machine running this dashboard must be on the same network as the gateway (able to reach `192.168.12.1`)
...
The gateway admin password is printed on the bottom label of your T-Mobile gateway device.
...
# Windows (CMD)
set DASHBOARD_PASSWORD=mypass && set GATEWAY_PASSWORD=gwpass && node server.js
```
...
| `GATEWAY_PASSWORD` | Yes | — | Admin password from the bottom of your gateway |
